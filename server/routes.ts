  ...
  app.post("/api/calculate", async (req, res) => {
    try {
      ...
      const submission = await storage.createSubmission({
        ...
        shipping: (calculations.shipping || 0).toString(),
        customsDuty: (calculations.customsDuty || calculations.duties || 0).toString(),
        gst: (calculations.gst || 0).toString(),
        lct: (calculations.lct || 0).toString(),
        inspection: (calculations.inspection || calculations.compliance || 0).toString(),
        serviceFee: (calculations.serviceFee || 0).toString(),
        totalCost: (calculations.totalCost || calculations.total || 0).toString(),
        serviceTier: calculations.serviceTier || "Standard",
...
async function getMarketPricingData(filters: {
  ...
  // Technical Intelligence API - Fixed routing with comprehensive engine data
  app.get("/api/vehicle-technical-intelligence", (req, res) => {
    ...
    console.log(`🔧 Technical Intelligence: ${make} ${model}`);
    ...
    console.log(`✅ Serving technical data for ${make} ${model}`);

    return res.json({
      vehicle: { make, model },
      technicalIntelligence: technicalData,
      source: 'comprehensive_engine_database',
